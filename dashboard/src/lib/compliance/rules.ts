...
import { checksumGTIN } from '../barcode'
import {
...
function runBarcode(d: EngineInputs): RuleCheck[] {
  ...
  const checksumOk = okLength ? checksumGTIN(digits) : null
  if (checksumOk === false) {
    return [rc('BARCODE', 'Barcode / GTIN', 'WARNING', {
      detected_value: digits,
      reason: `Barcode "${digits}" is standard length but the checksum check failed — the code may be misread or damaged. Please verify it manually.`,
      weight: 1,
    })]
  }
  return [rc('BARCODE', 'Barcode / GTIN', okLength ? 'PASS' : 'WARNING',
    {
      detected_value: digits,
      reason: okLength ? `Barcode "${digits}" is a standard EAN/GTIN length${checksumOk ? ' and its checksum digit validates' : ''}.` : `Barcode "${d.barcode}" is not a standard EAN-8/12/13/GTIN format.`,
      weight: 1,
    })]
}
...
export const RULES: EvaluatedRule[] = [
  ...
  },
  {
    ...
    applies: (ctx) => ctx.package_type === 'retail' || ctx.package_type === 'wholesale',
    evaluate: (_ctx, _d) => [rc('Rule 18', 'Wholesale/retail dealer obligations (Rules 18-21)', 'REQUIRES_PHYSICAL_INSPECTION',
  ...
  },
  {
    ...
    applies: (ctx) => ctx.package_type === 'retail' || ctx.package_type === 'wholesale',
    evaluate: (_ctx, d) => [rc('Rule 22', 'Maximum permissible error (First Schedule)', 'REQUIRES_PHYSICAL_INSPECTION',
  ...
  },
  {
    ...
    applies: () => true,
    evaluate: (ctx, _d) => ctx.applicable_exemptions.length > 0
