...
exports.lookupBarcode = exports.syncClaimsOnUserStatus = exports.setClaims = exports.complianceAssistant = exports.scanAnalysis = void 0;
const params_1 = require("firebase-functions/params");
const admin = __importStar(require("firebase-admin"));
const https_1 = require("firebase-functions/v2/https");
const firestore_1 = require("firebase-functions/v2/firestore");
const extraction_1 = require("./compliance/extraction");
const engine_1 = require("./compliance/engine");
admin.initializeApp();
const groqKey = (0, params_1.defineSecret)('GROQ_API_KEY');
const GROQ_VISION_MODEL = process.env.GROQ_VISION_MODEL ?? 'llama-3.2-11b-vision-preview';
const GROQ_TEXT_MODEL = process.env.GROQ_TEXT_MODEL ?? 'llama-3.3-70b-versatile';
const GROQ_ENDPOINT = 'https://api.groq.com/openai/v1/chat/completions';
function langName(lang) {
    return extraction_1.SUPPORTED_LANGS[lang] ?? 'English';
}
/* ------------------------------------------------------------------ */
...
async function assertCallerAuthorized(context) {
    if (!context.auth)
        throw new https_1.HttpsError('unauthenticated', 'You must be signed in.');
    const profile = await getProfile(context.auth.uid);
    if (!profile)
        throw new https_1.HttpsError('permission-denied', 'Account profile not found.');
    if (!['user', 'inspector', 'admin', 'super_admin'].includes(profile.role)) {
        throw new https_1.HttpsError('permission-denied', 'Invalid account role.');
    }
    if (profile.status === 'pending') {
        throw new https_1.HttpsError('permission-denied', 'Your staff access request is awaiting approval.');
    }
    if (profile.status !== 'active') {
        throw new https_1.HttpsError('permission-denied', 'Your account has been restricted. Contact the system administrator.');
    }
    return { uid: context.auth.uid, role: profile.role };
}
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
async function groqChat(model, system, parts) {
    ...
    try {
        ...
        try {
            ...
            console.error('Groq unreachable', e2.message);
            throw new https_1.HttpsError('unavailable', 'AI service is unreachable right now. Check your connection and retry.');
        }
        console.warn('Groq fetch retried after failure:', err.message);
    }
    if (!res.ok) {
        const text = await res.text();
        if (res.status === 429 || res.status === 403) {
            throw new https_1.HttpsError('resource-exhausted', 'AI service is rate limited. Wait a minute and retry.');
        }
        if (res.status === 400) {
            throw new https_1.HttpsError('invalid-argument', 'The AI service could not read this image — it may be too large or corrupted. Retake a closer, well-lit photo and retry.');
        }
        console.error('Groq error', res.status, text.slice(0, 500));
        throw new https_1.HttpsError('internal', 'AI service error. Try a clearer image.');
    }
    const payload = (await res.json());
    const content = payload.choices?.[0]?.message?.content ?? '';
    if (!content)
        throw new https_1.HttpsError('internal', 'Empty AI response. Try a clearer image.');
    return content;
}
async function imageToDataUrl(url) {
    const res = await fetch(url, { headers: { Origin: 'firebase' } });
    if (!res.ok)
        throw new https_1.HttpsError('failed-precondition', 'Could not fetch the uploaded image.');
    const buf = Buffer.from(await res.arrayBuffer());
    if (buf.length > 10 * 1024 * 1024)
        throw new https_1.HttpsError('failed-precondition', 'Image is larger than 10 MB.');
    const mime = res.headers.get('content-type') ?? 'image/jpeg';
    return `data:${mime};base64,${buf.toString('base64')}`;
}
/** Save an OCR-only scan (extraction failed) for inspector review. */
async function saveOcrFallback(actor, input, ocrText) {
    ...
    await scanRef.set({
        ...
        category: 'Other',
        barcode: (0, extraction_1.norm)(input.barcode) ?? '',
        overall_score: 0,
...
/* ------------------------------------------------------------------ */
exports.scanAnalysis = (0, https_1.onCall)({ secrets: [groqKey], timeoutSeconds: 120, memory: '1GiB' }, async (request) => {
    const actor = await assertCallerAuthorized(request);
    const input = request.data;
    const rawImages = input.images ?? [];
    if (input.image)
        rawImages.push(typeof input.image === 'string' ? { data: input.image } : input.image);
    if (rawImages.length === 0)
        throw new https_1.HttpsError('invalid-argument', 'No image provided.');
    if (rawImages.length > 6)
        throw new https_1.HttpsError('invalid-argument', 'At most 6 photos per inspection.');
    /* Stage 1 — OCR + transcription ONLY (Groq vision). */
    const parts = [{ type: 'text', text: (0, extraction_1.buildExtractionPrompt)(input.lang ?? 'en') }];
    for (const img of rawImages) {
        let url = img.data ?? '';
        if (!url && img.url)
            url = await imageToDataUrl(img.url);
        if (!url)
            continue;
    ...
    if (parts.length === 1)
        throw new https_1.HttpsError('invalid-argument', 'No valid image data provided.');
    const text = await groqChat(GROQ_VISION_MODEL, 'You are a machine. Output strict JSON only, no commentary.', parts);
    let raw = (0, extraction_1.extractJson)(text);
    let extractionStatus = 'ok';
    if (!raw || typeof raw !== 'object') {
        // One retry with an explicit "no markdown" reminder before giving up.
        const retryText = await groqChat(GROQ_VISION_MODEL, 'You are a machine. Output ONLY one JSON object — no markdown fences, no commentary, no trailing text.', parts);
        const raw2 = (0, extraction_1.extractJson)(retryText);
        if (!raw2 || typeof raw2 !== 'object') {
            extractionStatus = 'failed';
            const scanId = await saveOcrFallback(actor, input, text);
            throw new https_1.HttpsError('internal', `AI extraction returned invalid data (scan ${scanId} saved as OCR-only for review). Retry for a full analysis.`);
        }
        raw = raw2;
    }
    const parsed = (0, extraction_1.parseRaw)(raw, {
    ...
    if (!parsed.ocrText.trim() && !hasExtraction) {
        throw new https_1.HttpsError('invalid-argument', 'No readable text could be detected on the label. Please retake the photo in good light and close up on the declarations block, then retry.');
    }
    ...
    const outcome = (0, engine_1.runComplianceEngine)(inputs);
...
/* ------------------------------------------------------------------ */
exports.complianceAssistant = (0, https_1.onCall)({ secrets: [groqKey], timeoutSeconds: 60 }, async (request) => {
    const actor = await assertCallerAuthorized(request);
    const scanId = String(request.data?.scanId ?? '');
    const question = String(request.data?.question ?? '').trim();
    const lang = String(request.data?.lang ?? 'en');
    if (!scanId)
        throw new https_1.HttpsError('invalid-argument', 'scanId is required.');
    const snap = await admin.firestore().doc(`scans/${scanId}`).get();
    if (!snap.exists)
        throw new https_1.HttpsError('not-found', 'Scan not found.');
    const data = snap.data() ?? {};
    if (data.user_id !== actor.uid && actor.role !== 'inspector' && actor.role !== 'admin' && actor.role !== 'super_admin') {
        throw new https_1.HttpsError('permission-denied', 'You cannot review this scan.');
    }
    const condensed = {
    ...
    const parts = [
        {
            type: 'text',
            text: `Here is the inspection data (JSON):\n${JSON.stringify(condensed)}\n\n` +
...
/* ------------------------------------------------------------------ */
exports.setClaims = (0, https_1.onCall)(async (request) => {
    const actor = await assertCallerAuthorized(request);
    if (actor.role !== 'admin' && actor.role !== 'super_admin') {
        throw new https_1.HttpsError('permission-denied', 'Only admins can manage claims.');
    }
    ...
    if (!['active', 'blocked', 'pending'].includes(nextStatus)) {
        throw new https_1.HttpsError('invalid-argument', 'status must be active, blocked or pending.');
    }
    ...
    if (target.exists && target.data()?.role === 'super_admin' && actor.role !== 'super_admin') {
        throw new https_1.HttpsError('permission-denied', 'Only a super admin can modify a super admin account.');
    }
...
exports.syncClaimsOnUserStatus = (0, firestore_1.onDocumentUpdated)('users/{uid}', async (event) => {
...
exports.lookupBarcode = (0, https_1.onCall)({ timeoutSeconds: 15 }, async (request) => {
    const actor = await assertCallerAuthorized(request);
    const barcode = String(request.data?.barcode ?? '').trim();
    if (!barcode)
        throw new https_1.HttpsError('invalid-argument', 'barcode is required.');
    if (!/^\d{8,14}$/.test(barcode))
        throw new https_1.HttpsError('invalid-argument', 'Barcode must be 8-14 digits.');
    try {
