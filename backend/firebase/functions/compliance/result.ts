/**
 * Aggregation, scoring and the final, explained verdict.
 *
 * Verdict priority:
 *   1. any FAIL among applicable rules            -> NON_COMPLIANT
 *   2. any critical (weight>=2) NOT_VERIFIABLE     -> REQUIRES_PHYSICAL_INSPECTION
...
export function computeCompliance(rules: RuleCheck[], uncertainCount: number): ComplianceSummary {
  ...
  // Verdict.
  let verdict: FinalVerdict = 'COMPLIANT'
  if (counts.failed > 0) {
  ...
  const byStatus = (st: RuleStatus) => rules.filter((r) => r.status === st)
...
export function buildAssistant(c: ComplianceSummary, ex: Record<string, string | null>, productName: string | null, languages: string[]): { summary: string; suggestions: string[] } {
  ...
  const all = { ...ex }
  if (all.net_quantity) suggestions.push(`Physically weigh/measure the package — the label declares "${all.net_quantity}".`)
  if (all.mrp) suggestions.push('Spot-check the MRP against the GST invoice and confirm "incl. of all taxes" wording.')
  if (all.best_before) suggestions.push('Verify the best-before/use-by line against the physical package and its shelf state.')
  if (all.consumer_care) suggestions.push('Check that the consumer-care contact is operational.')
  if (all.manufacturer || all.packer || all.importer) suggestions.push('Cross-verify the printed business name/address against company registration.')
  if (languages.length > 0) suggestions.push('Confirm the mandatory declarations are printed in Hindi/English (Rule 6(2)).')
  if (c.failed.length > 0) suggestions.push(`Resolve the missing declarations: ${c.failed.join(', ')}.`)
  if (c.counts.not_verifiable > 0) suggestions.push(`${c.counts.not_verifiable} value${c.counts.not_verifiable > 1 ? 's' : ''} could not be read confidently — a human inspector should verify them on the physical label.`)
  if (c.counts.requires_physical_inspection > 0) suggestions.push(`Physical inspection is required for: ${c.requires_physical_inspection.join('; ')}.`)
  if (suggestions.length === 0) suggestions.push('Retake closer, well-lit photos of the back label for a more complete reading.')
