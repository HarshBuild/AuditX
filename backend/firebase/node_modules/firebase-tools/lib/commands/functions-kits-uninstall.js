...
async function handleInstance(options, config) {
    ...
    const projectNamesInEnvs = envFileNames.map((envName) => options.rc ? options.rc.resolveAlias(envName) : envName);
    ...
    const envFilesWhichAliasToProject = envFileNames.filter((name) => (options.rc?.resolveAlias(name) || name) === projectId);
    if (envFilesWhichAliasToProject.length > 1) {
        throw new error_1.FirebaseError(`Instance ${instanceId} contains multiple .env files which ambiguously resolve to the current project ${envFilesWhichAliasToProject.map((s) => ".env." + s).join(", ")}`);
    }
    if (envFilesWhichAliasToProject.length === 1) {
        filesystemNameForProject = envFilesWhichAliasToProject[0];
    }
    await uninstallProjectInstance(options, config, filesystemNameForProject, instanceId, instanceConfigDirPath);
}
async function uninstallInstance(options, config, instanceId, instanceConfigDirPath, onlyDeleteEmpty) {
    ...
    for (const fileName of fileNames) {
        ...
        await uninstallProjectInstance(options, config, projectIdOrAlias, instanceId, instanceConfigDirPath);
    }
...
async function uninstallProjectInstance(options, config, envName, instanceId, kitInstancePath) {
    const projectId = options.rc?.resolveAlias(envName) || envName;
    const envFilePath = (0, path_1.join)(kitInstancePath, `.env.${envName}`);
    if (!config.projectFileExists(envFilePath)) {
        throw new error_1.FirebaseError(`Expected to clean up project kit instance .env file at ${envFilePath}, but it doesn't exist.`);
    }
    const epFilters = [{ codebase: instanceId }];
    const deployContext = { projectId: projectId, filters: epFilters };
    const deletionCount = await (0, delete_1.deleteFunctionsByEndpointFilters)(deployContext, options);
    if (deletionCount === 0) {
